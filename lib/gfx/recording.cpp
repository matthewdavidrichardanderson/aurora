...
struct FrameRecorder {
  ...
  Vec2<uint32_t> offscreenLogicalSize;
...
void finish_current_offscreen() {
  ...
  g_recorder.offscreenLogicalSize = {};
}

void start_offscreen(uint32_t width, uint32_t height, uint32_t logicalWidth, uint32_t logicalHeight) {
...
Vec2<uint32_t> get_logical_render_target_size() noexcept {
  if (g_recorder.inOffscreen && g_recorder.offscreenLogicalSize.x != 0 && g_recorder.offscreenLogicalSize.y != 0) {
    return g_recorder.offscreenLogicalSize;
  }
...
void begin_offscreen(uint32_t width, uint32_t height, uint32_t logicalWidth, uint32_t logicalHeight) {
  ZoneScoped;
  ...
  start_offscreen(width, height, logicalWidth, logicalHeight);
}
